...
v_n=[A(i,3);A(i,4)]/sqrt(A(i,4)^2+A(i,3)^2);
    ...
    for j=1:size(A,1)     %iteration for every agents
        if(i~=j) %
        d=sqrt((A(i,1)-A(j,1))^2+(A(i,2)-A(j,2))^2);
            if d<sight_radius %calculates force only if distance to the other agent is smaller than sight_radius
                ...
                FY=FY+A1*exp((r_agents-d)/B1)*n(2)*(lambda+(1-lambda)*(1+cos(phi))/2)+A2*exp((r_agents-d)/B2)*n(2);
            end
        end
    end
end